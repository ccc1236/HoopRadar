...
- **First use pulls fresh data, so it takes a few seconds.** fNBA caches nba.com stats for about 6 hours. Each Window (Season / L5 / L10) and Per Mode (Per Game / Per 36 / Per 100) is cached separately, so the first time you pick a new combination it refetches, and opening a player's spider chart loads all three windows at once. While that happens you will see a pulsing "Loading..." or "Refreshing..." status. Requests are deliberately spaced out to stay friendly with nba.com, which is why a cold fetch is not instant. Subsequent views inside the cache window are immediate.
- **nba.com is occasionally flaky.** It sometimes returns a brief error on a cold request and then serves it fine moments later. fNBA retries automatically; if a stat still fails to load, clicking Refresh usually clears it.
