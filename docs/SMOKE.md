...
```js
await hoopradar.getPlayerStats({
  type: "getPlayerStats",
  yahooIds: ["unknown"],
  window: "Season",
  perMode: "PerGame",
...
call `saveMapping` via the `hoopradar` debug object instead:

```js
await hoopradar.saveMapping("2025-26", [
...
await hoopradar.cache.get("lps:2025-26:Base:PerGame:0"); // returns the cached rows array
```
...
- A horizontal HoopRadar filter bar appears above the stats table.
...
- Yahoo's native stat-range filter is greyed out with "(HoopRadar active)" appended.
...
- Load the My Team page fresh. By default Yahoo opens "Stats > Today", which shows live game-day numbers; HoopRadar should NOT overlay there.
- Confirm: no filter bar on the table; instead a small banner reads "HoopRadar shows on Average Stats > {season} Season. Click to switch." with a Switch tab button.
- Click the Switch tab button. Yahoo should swap to Average Stats > {season} Season, and the banner should be replaced by the full HoopRadar filter bar with the overlaid columns.
...
Same overlay applies. Bench and starter rows both get HoopRadar columns.
...
```js
await hoopradar.loadMapping("2025-26");
```
